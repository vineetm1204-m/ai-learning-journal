...
Uses OpenRouter API to generate structured Deep Learning journal entries.
"""
...
def get_client() -> OpenAI:
    ...
    if not api_key:
        raise JournalGenerationError(
            "OPENROUTER_API_KEY is missing or empty. Set the environment variable "
...
def generate_content(client: OpenAI, *, prompt: str, temperature: float):
    try:
        ...
        if "429" in message or "rate limit" in message.lower():
            raise JournalGenerationError(
                "OpenRouter quota was exceeded for this API key. Check your plan or billing, then run the script again."
            ) from exc
        raise
